...
public class ConnectionPoolContextListener implements ServletContextListener {
  ...
  private DataSource createConnectionPool() {
    ...
    config.setJdbcUrl(String.format("jdbc:mysql:///%s", "betaplayaddition"));
    
    config.setUsername("betaplayaddition"); // e.g. "root", "postgres"
    config.setPassword("betaplayaddition"); // e.g. "my-password"
    

    // For Java users, the Cloud SQL JDBC Socket Factory can provide authenticated connections.
    // See https://github.com/GoogleCloudPlatform/cloud-sql-jdbc-socket-factory for details.
    config.addDataSourceProperty("socketFactory", "com.google.cloud.sql.mysql.SocketFactory");
    config.addDataSourceProperty("cloudSqlInstance", "betaplayaddition:us-west2:betaplayaddition");
